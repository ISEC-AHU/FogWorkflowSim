...
public class TSPSchedulingAlgorithm extends BaseSchedulingAlgorithm {
    ...
    public void run() {
        ...
        for (Iterator it = cloudletList.iterator(); it.hasNext();) { //Task list
            ...
            if (next.getTaskList().size() > 0){
                ...
                if (CloudSim.clock() >= task_info.getTimeSubmission() && (!Parameters.getConsiderTasksParallelismRestrictions() || TSPJobManager.canRunTask(task_info.getJobId(), task_info.getTaskId()))) {
    ...
    public boolean placer(Cloudlet cloudlet, TSPTask taskInfo){
        ...
        if (action != -1){
            ...
            double task_running_time = taskInfo.getMi() / vm.getMips();
            ...
            if (Parameters.isDeadlinePenalizationEnabled() &&  task_running_time + CloudSim.clock() > taskInfo.getTimeDeadlineFinal()){
                ...
                if (Parameters.getPrioritiesQuantity() > 0){
                    cost *= taskInfo.getPriority() * 1.0 / Parameters.getPrioritiesQuantity();
                }

            }else {
                switch (Parameters.getOptimization()) {
                    case AvgTaskCompletionTime:
                        cost = TSPJobManager.getAvgTaskCompletionTime(taskInfo.getTimeSubmission() + task_running_time);
                        break;
                    case TaskRunningTime:
                        cost = TSPJobManager.getAvgTaskRunningTime(task_running_time);
                        break;
                    case Energy:
                        cost = TSPJobManager.getEnergyConsumption(not_mobile_list);
                        break;
                    case TaskEnergy:
                    ...
                    case TaskCompletionTimeAndEnergy:
                        double w1 = 0.5;
                        double w2 = 0.5;
                        cost = (taskInfo.getTimeSubmission() + task_running_time) * w1;
                        cost += TSPJobManager.getEnergyConsumption(not_mobile_list) * w2;
                        break;
                    default:
            ...
            TSPSocketClient.saveReward(taskInfo.getCloudletId(), - cost);
